---
title: " AnalyzAIRR- Comprehensive AIRR data analysis"
author:
...
# Installation
The latest release of **AnalyzAIRR** can be installed directly from its github repository:
```{r github1, echo = TRUE, eval = FALSE}
#AnalyzAIRR dependencies installation
list.pkgs <- c("data.table", "pbapply", "pheatmap", "DESeq2", "Rcpp", "vegan", "ggplot2","ade4","directlabels", "ggVennDiagram", "plyr", "reshape2", "ggprism", "eulerr", "ggpmisc", "circlize","naturalsort", "scales", "magick", "dplyr", "ggsci",  "gridextra", "ggpubr")

pkgs <- list.pkgs[!(list.pkgs %in% installed.packages()[,"Package"])]
if(length(pkgs)>0) install.packages(pkgs)

#AnalyzAIRR package installation
install.packages(devtools)  
devtools::install_github("pham-ph/RepSeq")
```
...
### Use input files in AnalyzAIRR-compliant formats
...
**2. Load the metadata**
  ...
  * a column with the sample names that match the name of the alignment files and their order in the list.
  This column is assigned to the `row.names` argument when loading the metadata file. Only one column containing the sample names should be provided in the metadata file.
...
+ **Filter sequences based on their count in each sample.**
...
+ **Exclude one or multiple samples from a RepSeqExperiment object.**
  ...
  `dropSample()` offers the possibility to filter out one or multiple repertoires by specifying their corresponding sample_id. 
...
# Exploratory statistics {#Exploratory}
...
+  **Rarefaction analysis** 
   ...
   Rarefaction curves can as well be plotted using `plotRarefaction()`

```{r plot rarefaction, echo=T, fig.width=8, fig.asp = 0.5,fig.cap="Figure 3: Rarefaction curves plotting the number of clonotypes against the number of sequences in a sample."}
...
+  **Evaluation of the Renyi diversity** 
   ...
   The Renyi index is a generalization of the Shannon entropy. It represents the distribution of clonal expansions as a function of the parameter alpha. It equally considers all species including the rare ones at alpha=0, and up-weighs the abundant species with an increasing value of alpha. Alpha =1 is an approximation of the Shannon index; alpha = 2 corresponds to the Simpson index and alpha=Inf corresponds to the Berger-Parker index. The latter highlights the highest clonal expansion in a repertoire.
...
```
  ...
  Similarly to the previous function, Renyi's entropy values can be plotted using `plotRenyiIndex()`. 

```{r plot renyi, echo = TRUE, fig.width=8, fig.height = 3.5,fig.cap="Figure 6: The Renyi values were calculated at the J gene level for each of the pre-defined alpha values and plotted for each sample."}
...
```{r intervals, fig.height=3.5, fig.width=8,fig.cap="Figure 7: Stacked bars showing the proportion (left plot) and the cumulative frequency (right plot) of clones within each count interval in each sample."}
...
```{r plot clone distri, echo = TRUE, warning=FALSE,fig.width=8, fig.height = 3.5,fig.pos="p",fig.cap="Figure 8: Plotted is the distribution of the clones as a function of their occurrence rank within each sample."}
...
```{r plotIndCountIntervals, echo=TRUE,message=FALSE, warning=FALSE,fig.width=8, fig.height = 4,fig.pos="p",fig.cap="Figure 9: Barplots showing the proportion (upper plot) and the cumulative frequency (bottom plot) of clones within each count interval for a selected sample."}
plotIndCountIntervals(RepSeqData,level = "clone")
```

* **Analyze V and J gene usages**

    `plotIndGeneUsage()`computes V and J gene usages. 
  The parameter `scale` allows to choose whether to compute the count or the frequency.
```{r J usage ind, echo = TRUE, message=F,warning=F,fig.height = 3,fig.width= 8, fig.cap="Figure 11: Barplots showing the frequency of each J gene in one selected sample."}
...
```
   ...
   Users can choose whether to plot the top most used combinations using the `prop` parameter or all combinations expressed in a sample.

```{r VJusage, message=F,warning=F,fig.width=5, fig.height=4, warning=F, message=F, fig.cap="Figure 12: A circos plot showing the 1% most frequent VJ combinations in a selected sample."}
#Plot the top 1% VJ combinations
plotVJusage(x=RepSeqData, scale = "frequency", prop=0.01)
```
...
```{r spectra, fig.width=8, fig.height = 3.5,fig.cap="Figure 13: Spectratyping for a selected sample. The 10% most frequent V genes are distinctively coloured. "}
#Show the top 10%
...
```{r spectraV, fig.height = 3.5,fig.width=8,fig.cap="Figure 14: Spectratyping per V gene for a selected sample. The 10% most frequent V genes are represented"}
...
The majority of the functions described in [this section](#Exploratory) can be applied to compare groups of samples and perform statistical tests.

+ **Compare metaData statistics**

   The calculated statistics in the **metaData** slot can be compared between groups of samples by specifying the `groupBy` parameter in the `plotStatistics()` function.

<div class="column-left">
```{r comp sd1,fig.cap="Figure 15: The number of sequences is not statistically different between the two studied cell subsets. Boxplots represent the median across all samples belonging to the same group. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown."}
plotStatistics(x = RepSeqData, 
               stat = "clonotype", 
               groupBy = "cell_subset")
```
...
<div class="column-right">
```{r comp sd2,fig.cap="Figure 16: The number of sequences is statistically different between the two studied cell subsets. Boxplots represent the median across all samples belonging to the same group. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown."}
plotStatistics(x = RepSeqData, 
               stat = "clonotype", 
               groupBy = "sex")
```
...
```{r J usage , echo = TRUE, fig.width=9, fig.height=3, message=F,warning=F,fig.align='center', fig.cap="Figure 17: Boxplots showing the frequency of each J gene within each group of samples. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown. Only significant values are plotted."}
...
+ **Compare repertoire diversities**
   ...
   For the Renyi index, the `grouped` parameter should be set to **TRUE**.

<div class="column-left">
```{r plot indices 3, fig.cap="Figure 18: The Gini index is calculated at the clone level and compared between cell subsets. Boxplots represent the median across all samples belonging to the same group. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown." }
plotDiversity(x = RepSeqData, 
              level = "clone", 
              groupBy = "cell_subset", 
              index = "gini")
...
<div class="column-right">
```{r plot indices 4, fig.cap="Figure 19: The Renyi values were calculated at the clone level for each of the pre-defined alpha values and plotted for each sample. For each group, the mean (circles) and stantard error (shade) are plotted. The area under the curve (AUC) is calculated for each sample and compared between chosen groups using a Wilcoxon test with Holm's correction." }
...
```{r plotDistribVpJall,fig.height = 3.5, fig.width=8, fig.pos="p",fig.cap="Figure 20: The proportion (left plot) and the cumulative frequency (right plot) of clones within each count interval was compared between amTreg and nTreg samples. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown."}
plotCountIntervals(x=RepSeqData, level="clone", groupBy = "cell_subset")
```
...
Different statistical methods are proposed herein to evaluate this convergence.
 ...
 + **Compute the number of shared sequences**
   ...
   The number of shared sequences, at any level, between samples belonging for instance to the same experimental group can be obtained using the `plotEulerr()` function.
  If `sampleNames` is not specified, the first 3 samples in the datasets will be analyzed.

```{r venn, fig.pos="p",echo = T, eval=T,  fig.width=8, fig.height=4,fig.cap="Figure 21: Euler diagram representing the number of shared clones between the nTreg samples."}
ctrnames <- rownames(mData(RepSeqData))[which(mData(RepSeqData)[,"cell_subset" ] %in% "nTreg")]
plotEulerr(x = RepSeqData, 
           level = "clone",
...
+ **Compute dissimilarity distances**
   ...
   **AnalyzAIRR** proposes a list of dissimilarity indices, each taking into account different parameters. The proposed methods include: 
   ...
   Details on these indices and others can be found [here](https://www.rdocumentation.org/packages/vegan/versions/2.4-2/topics/vegdist)

  These distances can be calculated at any level of the repertoire and can be:

    - Visualized as a dissimilarity heatmap using `plotDissimilarityMatrix()`.
  This function performs a hierarchical clustering on the calculated distance scores using the method specified in  the `clustering` parameter.
    - Used to perform a multidimensional scaling analysis or a principal component analysis using `plotDimReduction()` with the `dim_method` parameter set to **MDS**

<div class="column-left">
```{r dissimilarity, echo = TRUE, fig.cap="Figure 24: A heatmap showing the Morisita distance calculated at the clone level between all pairwise samples. The ward.D method was used to perform the hierarchical clustering."}
plotDissimilarityMatrix(x = RepSeqData, 
                        level = "clone", 
                        method = "morisita", 
                        clustering = "ward.D",
                        binary = "FALSE")
...
<div class="column-right">
```{r mds1, echo = TRUE,warning=FALSE, message=FALSE, fig.cap="Figure 25: The Morisita distance were calculated at the clone level between all pairwise samples within the dataset and used to perform a multidimensional scaling analysis."}
...
`diffExpGroup` is integrated in the:

   - `plotVolcano()` function which can be directly applied to a RepSeqExperiment object, allowing an easy visualization of the results in a volcano plot.
  ...
  - `plotDimReduction()` function with the `dim_method` parameter set to **PCA** which performs a principal component analysis.
    
<div class="column-left">
```{r volcano, echo = TRUE, warning=FALSE, message=FALSE,fig.cap="Figure 26: A volcano plot showing differentially expressed VJ combinations between amTreg and nTreg samples. Over-expressed combinations in the amTreg population are shown in red."}
...
```{r pca1, echo = TRUE,warning=FALSE, message=FALSE,fig.cap="Figure 27: A principal component analysis is performed on the differential VJ expression analysis between amTreg and nTreg samples. Samples are colored based on the cell subset." }
plotDimReduction(x = RepSeqData, 
                 level = "VJ", 
                 colorBy = "cell_subset", 
                 dim_method = "PCA")
...
```{r, echo=T,warning=F, message=F, fig.width=8, fig.height=4, fig.cap="Figure 28: A heatmap plotting the perturbation scores of the CDR3aa length distribution within each V gene in the amTreg population compared to the nTregs (used as reference). The parameter order allows to define the order in which the samples will be plotted on the heatmap which helps a better separation between the control groups and the other samples."}
