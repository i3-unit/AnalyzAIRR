...
```
<figcaption align = "center"><em class="special">Figure 3: Rarefaction curves plotting the number of clonotypes against the number of sequences in a sample.</em></figcaption>
<br>
...
```
<figcaption align = "center"><em class="special">Figure 6: The Renyi values were calculated at the J gene level for each of the pre-defined alpha values and plotted for each sample.</em></figcaption>
...
```{r intervals, fig.height=4, fig.width=8,fig.align="center"}
plotCountIntervals(x=RepSeqData, level="clone")
```
<figcaption align = "justify"><em class="special">Figure 7: Stacked bars showing the proportion (left plot) and the cumulative frequency (right plot) of clones within each count interval in each sample.</em></figcaption>
...
```
<figcaption align = "center"><em class="special">Figure 8: Plotted is the distribution of the clones as a function of their occurrence rank within each sample.</em></figcaption>
...
```{r plotIndCountIntervals, echo=TRUE,message=FALSE, warning=FALSE,fig.width=8, fig.height = 4,fig.align="center", cache.comments=FALSE}
plotIndCountIntervals(RepSeqData,level = "clone")
```
<figcaption align = "center"><em class="special">Figure 9: Barplots showing the proportion (upper plot) and the cumulative frequency (bottom plot) of clones within each count interval for a selected sample.</em></figcaption>
...
```
<figcaption align = "center"><em class="special">Figure 11: A circos plot showing the 1% most frequent VJ combinations in a selected sample.</em></figcaption>
<br>
+ **CDR3 Spectratyping**
...
```
<figcaption align = "center"><em class="special">Figure 12: Spectratyping for a selected sample. The 10% most frequent V genes are distinctively coloured.</em></figcaption>
<br>
...
```
<figcaption align = "center"><em class="special">Figure 13: Spectratyping per V gene for a selected sample. The 10% most frequent V genes are represented.</em></figcaption>
...
<div class="column-left">
```{r comp sd1,fig.cap="Figure 14: The number of sequences is not statistically different between the two studied cell subsets. Boxplots represent the median across all samples belonging to the same group. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown."}
plotStatistics(x = RepSeqData,
               stat = "clonotype",
               groupBy = "cell_subset")
```
...
<div class="column-right">
```{r comp sd2,fig.cap="Figure 15: The number of sequences is statistically different between the two studied cell subsets. Boxplots represent the median across all samples belonging to the same group. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown."}
plotStatistics(x = RepSeqData,
               stat = "clonotype",
               groupBy = "sex")
```
...
```{r J usage , echo = TRUE, fig.width=9, fig.height=4, message=F,warning=F,fig.align="center"}
plotGeneUsage(x=RepSeqData, level="J", scale="frequency", groupBy="cell_subset")
```
<figcaption align = "center"><em class="special">Figure 16: Boxplots showing the frequency of each J gene within each group of samples. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown. Only significant values are plotted.</em></figcaption>
<br>
...
```{r plotDistribVpJall,fig.height = 4, fig.width=8, fig.align='center'}
plotCountIntervals(x=RepSeqData, level="clone", groupBy = "cell_subset")
```
<figcaption align = "center"><em class="special">Figure 19: The proportion (left plot) and the cumulative frequency (right plot) of clones within each count interval was compared between amTreg and nTreg samples. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown.</em></figcaption>
...
```
<figcaption align = "center"><em class="special">Figure 20: Euler diagram representing the number of shared clones between the nTreg samples.</em></figcaption>
<br>
...
<div class="column-left">
```{r scatter, echo = T, eval=T, message=FALSE, fig.cap="Figure 21: A biplot comparing the V gene usage between two defined samples. A linear model is fitted on the data. The V usage is significantly similar between the two samples."}
...
<div class="column-right">
```{r scatter 2, echo = T, eval=T, message=FALSE, fig.cap="Figure 22: A biplot comparing the clonal expression between two defined samples. A linear model is fitted on the data. Clonal overlap and expression are significantly low between the compared samples."}
plotScatter(x = RepSeqData,
            level = "clone",
            scale = "frequency",
            sampleNames = c("tripod-30-813","tripod-31-846"))
...
<div class="column-left">
```{r dissimilarity, echo = TRUE, fig.cap="Figure 23: A heatmap showing the Morisita distance calculated at the clone level between all pairwise samples. The ward.D method was used to perform the hierarchical clustering."}
...
<div class="column-right">
```{r mds1, echo = TRUE,warning=FALSE, message=FALSE, fig.cap="Figure 24: The Morisita distance were calculated at the clone level between all pairwise samples within the dataset and used to perform a multidimensional scaling analysis."}
...
<div class="column-left">
```{r volcano, echo = TRUE, warning=FALSE, message=FALSE,fig.cap="Figure 25: A volcano plot showing differentially expressed VJ combinations between amTreg and nTreg samples. Over-expressed combinations in the amTreg population are shown in red."}
...
```{r pca1, echo = TRUE,warning=FALSE, message=FALSE,fig.cap="Figure 26: A principal component analysis performed on the differential expression analysis between amTreg and nTreg samples on VJ combinations. Samples are colored based on the cell subset." }
plotDimReduction(x = RepSeqData,
                 level = "VJ",
                 colorBy = "cell_subset",
                 dim_method = "PCA")
