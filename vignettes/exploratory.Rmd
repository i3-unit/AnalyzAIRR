...
Together, the Chao1 index and rarefaction curves serve complementary roles. Rarefaction curves assess the sufficiency of sampling effort and provide a visual representation of species accumulation, while the Chao1 index estimates the total species richness by considering rare species.
...
renyiIndex(x = RepSeqData, 
           level = "aaClone")

```

```{r view renyi, echo = F, verbose=FALSE}
rn<- renyiIndex(x = RepSeqData, 
                level = "aaClone")
...
```
  ...
  Renyi's entropy values can be plotted using `plotRenyiIndex()`. A curve for each sample is plotted and colors are attributed based on the `colorBy` parameter. 
...
```
<figcaption align = "center"><em class="special">Figure 5: The Renyi values were calculated at the J gene level at each pre-defined alpha value and plotted for each sample.</em></figcaption>
...
plotIntervals(x = RepSeqData, 
              ...
              facetBy = "cell_subset")
```
<figcaption align = "center"><em class="special">Figure 6: Stacked bars showing the distribution (upper plot) and the cumulative frequency (lower plot) of clones within each count interval in each sample.</em></figcaption>
<br>
