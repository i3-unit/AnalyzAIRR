...
<div class="column-left">
```{r comp sd1,fig.cap="Figure 14: The number of clonotypes is not statistically different between the two studied cell subsets. Boxplots represent the median across all samples belonging to the same group. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown."}
plotStatistics(x = RepSeqData,
               stat = "clonotype",
               groupBy = "cell_subset")
```
...
<div class="column-right">
```{r comp sd2,fig.cap="Figure 15: The number of clonotypes is not statistically different between the two studied cell subsets within each sex. Boxplots represent the median across all samples belonging to the same specified group. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown."}
plotStatistics(x = RepSeqData,
               stat = "clonotype",
...
```{r plot plotRenyiIndexG, fig.height = 3.5, fig.width=8, fig.align='center'}
...
```
<figcaption align = "center"><em class="special">Figure 20: The proportion (left plot) and the cumulative frequency (right plot) of clones within each count interval was compared between amTreg and nTreg samples within each sex. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown.</em></figcaption>
...
```
<figcaption align = "center"><em class="special">Figure 21: Euler diagram representing the number of shared clones between the nTreg samples.</em></figcaption>
<br>
...
The results can be visualized using the `plotDiffExp` function:

   - into a volcano plot, by setting `plot` to **Volcano**.
  ...
  - by performing a principal component analysis by setting `plot` to **PCA**. In this case, only the `group` parameter should be specified. 
...
```{r pca1, echo = TRUE,warning=FALSE, message=FALSE,fig.cap="Figure 27: A principal component analysis performed on the differential expression analysis between amTreg and nTreg samples on VJ combinations. Samples are colored based on the cell subset." }
plotDiffExp(x = RepSeqData,
            level = "VJ",
            group = c("cell_subset", "amTreg", "nTreg"),
            plot = "PCA"
