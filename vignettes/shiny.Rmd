...
Alternatively, **Shiny-AnalyzAIRR** has been deployed on the web and can be used directly at [this link](https://i3-analyzairr.fr).
...
The metadata should be provided as a table containing:
  ...
  * a column `sample_id` with the sample names that match the name of the alignment files and their order.
  ...
  * any additional columns with relevant information for the analyses. Columns could encompass the experimental conditions, clinical variables, etc... 
  ...
  Besides `sample_id`, no specific column names nor order are required.
