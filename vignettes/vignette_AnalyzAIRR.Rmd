...
+  **Rarefaction analysis** 
   ...
   Rarefaction curves can as well be plotted using `plotRarefaction()`. The `colorBy` parameter is used to assign colors to plotted curves based on any group column from the **metaData** slot, and `facetBy` to separate the samples based on a different set of group columns.
...
```
 ...
 The `facetBy` parameter can be used to separate samples based on a group column. A shape can also be assigned for each curve of groups of curves with the `shapeBy` parameter.
...
+ **Assess the clonal distribution per interval**
   ...
   `plotIndIntervals()` allows the analysis of the clonal distribution per intervals of counts or fractions in a single sample.

```{r plotIndIntervals, echo=TRUE,message=FALSE, warning=FALSE,fig.width=7, fig.height = 4,fig.align="center", cache.comments=FALSE}
plotIndIntervals(RepSeqData,level = "aaClone", sampleName="tripod-30-813", fractions="frequency")
```
<figcaption align = "center"><em class="special">Figure 9: Barplots showing the distribution (upper plot) and the cumulative frequency (lower plot) of aaClone within each frequency interval for one selected sample.</em></figcaption>
<br>
