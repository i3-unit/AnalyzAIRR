...
![](/Users/vanessamhanna/Github/generate-AnalyzAIRR/vignettes/graphical.png)
...
A list of 8 samples were selected from a published study (Mhanna et al., 2021) to illustrate the use of this package. Fastq files were aligned using MiXCR and exported in a MiAIRR format, and can be found at https://github.com/tchitchek-lab/RepSeq/extdata/mixcr
...
getPublic(x=RepSeqData, level="aaClone", group=c("cell_subset", "amTreg"))
```
...
**Note**: All the above-mentioned functions can be applied at any of the following repertoire level: **aaClone**, **ntClone**, **aaCDR3** and **ntCDR3**.
...
This strategy adapted in (Chaara et al., 2018) can be used to eliminate “uninformative” sequences resulting from experimental noise. It is particularly efficient when applied on small samples as it corrects altered count distributions caused by a high-sequencing depth.
...
Statistics in the **metaData** slot, calculated during the building of the RepSeqExperiment object, can be visualized for each sample using the `plotStatistics()` function with the `colorBy` parameter set to the sample id column. Groups can be separated using the `facetBy` parameter. 
...
```{r sd2,fig.cap="Figure 2: A barplot showing the number of ntClones identified in each sample."}
plotStatistics(x = RepSeqData,
               stat = "ntClone",
               colorBy = "sample_id")
```
...
To evaluate the clonotype richness and the sequencing depth of a sample, a rarefaction curve can be plotted, which illustrates the relationship between the number of sequences randomly selected from the sample and the number of clones they represent. A plateau in the curve suggests that sampling is yielding few or no new species, and that the observed clonotype richness is approaching the true richness in the sequenced sample. In contrast, a steep ascending curve indicates that additional sequencing may still yield new clones.
  
  Rarefaction curves can be plotted using `plotRarefaction()`. The `colorBy` parameter is used to assign colors to plotted curves based on any group column from the **metaData** slot, and `facetBy` to separate the samples based on a different set of group columns.
...
To calculate the number of clones that are not observed in the sample, the Chao index can be used, which estimates the total clone richness of a sample including clones that are likely present but unseen or undetected. 
Together, the Chao1 index and rarefaction curves serve complementary roles. Rrarefaction curves assess the sufficiency of sampling effort and provide a visual representation of species accumulation, while the Chao1 index estimates the total species richness by considering rare species.

The Chao index, which values are already available in the **metaData** slot, can be plotted using the `plotStatScatter()` function. This function plots the values of two statistics along two axes, showing the relationship between them. Chao values can thus be plotted against the number of observed ntClones for instance, to evaluate the difference in the number of observed and unseen clones. 

```{r plot plotStatScatter,  echo=T, fig.width=6,fig.height=4, fig.align="center"}
plotStatScatter(x = RepSeqData, stat1 = "chao1", stat2 = "ntClone", colorBy = "sample_id")
...
diversityIndices(x=RepSeqData, level="aaClone")

```
```{r indices bis, echo = FALSE}
indices<- diversityIndices(x=RepSeqData, level="aaClone")
```
...
<div class="column-left">
```{r plot indices 1,verbose=FALSE, fig.cap="Figure 4: A barplot showing the Gini values calculated at the aaClone level within each sample."}
...
<div class="column-right">
```{r plot indices 2,fig.cap="Figure 5: A barplot showing the Shannon values calculated at the ntClone level within each sample."}
plotDiversity(x=RepSeqData,
              level = "ntClone",
              index = "shannon",
               colorBy="sample_id")
...
renyiIndex(x=RepSeqData, level="aaClone")

```
```{r renyi bis, echo = FALSE}
rn<- renyiIndex(x=RepSeqData, level="aaClone")
```
...
```
 ...
 The `facetBy` parameter can be used to separate samples based on a group column.

```{r plot renyi, echo = TRUE, fig.width=8, fig.height = 4,fig.align="center"}
plotRenyiIndex(x=RepSeqData, level="J", grouped=FALSE, colorBy = "sample_id", facetBy="cell_subset")
```
...
+ **Clonal distribution within count intervals**
  ...
  The `plotIntervals()` function can be used to assess the clonal distribution within pre-defined count intervals.
...
plotIntervals(x=RepSeqData, level="aaClone", fractions="frequency", facetBy = "cell_subset")
```
...
plotRankDistrib(x=RepSeqData,level="aaClone", colorBy="sample_id", scale="frequency", ranks=1000, grouped=F)
```
...
+ **Evaluate basic and diversity statistics**

 The `plotIndStatistics()` function allows the positioning of a sample of interest within the whole dataset. Basic or diversity indices are plotted for the whole dataset, where the sample of interest is highlighted as a red cross, allowing for the comparison of the sample to the whole dataset, and for the potential identification of sample outliers. 

<div class="column-left">
```{r plotIndStatistics1, fig.cap="Figure 14: Boxplots showing metadata statistics calculated on all the dataset samples. The sample of interest is represented by a red cross"}
plotIndStatistics(x = RepSeqData, stat = "metadata",  level = "aaClone")

```
</div>
<div class="column-right">
```{r plotIndStatistics2, fig.cap="Figure 15: Boxplots showing different diversity indices calculated on all the dataset samples. The sample of interest is represented by a red cross."}
...
+ **Assess the clonal distribution per interval**

   To visualize the repertoire structure and the top clone distribution in a repertoire, `plotIndMap()` can be used to plot a circular treemap in which each circle represents a unique clone, and the circle size corresponds to the clone count. Users can choose the proportion of the top clones to plot using the `prop` parameter. We recommend to plot up to the top 5% of clones, as higher values might generate difficult-to-Interpret Visuals.
   
```{r plotIndMap, echo=TRUE,message=FALSE, warning=FALSE,fig.width=4, fig.height = 4,fig.align="center"}
plotIndMap(x=RepSeqData, sampleName=NULL, level="aaClone", prop=0.01)
```
<figcaption align = "center"><em class="special">Figure 9: A circular treemap was generated for one single sample and shows the structure of the top 1% of aaClones in the sample repertoire. Each circle represents a unique aaClone, and the circle size corresponds to the aaClone count.</em></figcaption>
<br>   
  
  To quantify these observations, the `plotIndIntervals()` function can be used which allow the analysis of the clonal distribution per intervals of counts or fractions in a single sample.

```{r plotIndIntervals, echo=TRUE,message=FALSE, warning=FALSE,fig.width=6, fig.height = 3,fig.align="center", cache.comments=FALSE}
plotIndIntervals(RepSeqData, level = "aaClone", sampleName="tripod-30-813", fractions="frequency")
```
<figcaption align = "center"><em class="special">Figure 10: Barplots showing the distribution (upper plot) and the cumulative frequency (lower plot) of aaClone within each frequency interval for one selected sample.</em></figcaption>
<br>

  To visualize the repertoire structure and the top clone distribution in a repertoire, `plotIndMap()` can be used to plot a circular treemap in which each circle represents a unique clone, and the circle size corresponds to the clone count.
...
plotIndGeneUsage(x = RepSeqData, level = "aaClone", sampleName = NULL)
```
<figcaption align = "center"><em class="special">Figure 11: Heatmap of V and J combination usage in one selected sample. Frequencies, represented by the color scale, are scaled column-wise. Barplots at the top and right side of the heatmap show the usage of each gene across the row or the column for the V and J genes, respectively.</em></figcaption>
<br>
...
<div class="column-left">
```{r comp sd1,fig.cap="Figure 14: Comparison of the the number of ntClones between amTreg and nTreg samples. Boxplots represent the median across all samples belonging to the same group. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown."}
plotStatistics(x = RepSeqData,
               stat = "ntClone",
               colorBy = "cell_subset",
               grouped=TRUE)
...
<div class="column-right">
```{r comp sd2,fig.cap="Figure 15: Comparison of the the number of ntClones between amTreg and nTreg samples within each sex. Boxplots represent the median across all samples belonging to the same specified group. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown."}
plotStatistics(x = RepSeqData,
               stat = "ntClone",
               colorBy = "cell_subset", facetBy="sex",
               grouped=TRUE)
```
</div>
...
plotGeneUsage(x=RepSeqData, level="J", scale="frequency", colorBy="cell_subset")
```
...
+ **Compare repertoire diversities**
   ...
   For The `plotDiversity()` function, the `grouped` parameter should be set to **TRUE**, and the groups to be analysed should be specified in the `colorBy` parameter.

<div class="column-left">
```{r plot indices 3,verbose=FALSE, fig.cap="Figure 17: The Gini index is calculated at the aaClone level and compared between cell subsets. Boxplots represent the median across all samples belonging to the same group. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown." }
plotDiversity(x = RepSeqData,
              level = "aaClone",
              index = "gini",
              colorBy = "cell_subset",
...
<div class="column-right">
```{r plot indices 4, verbose=FALSE, fig.cap="Figure 18: The Gini index is calculated at the aaClone level and compared between cell subsets within each sex. Boxplots represent the median across all samples belonging to the same group. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown." }
plotDiversity(x = RepSeqData,
              level = "aaClone",
              index = "gini",
              colorBy = "cell_subset", facetBy = "sex",
...
plotIntervals(x=RepSeqData, level="aaClone", colorBy = "cell_subset", facetBy="sex", fractions="count", grouped=TRUE)
```
<figcaption align = "center"><em class="special">Figure 20: The distribution (upper plot) and the cumulative frequency (bottom plot) of aaClones within each count interval was compared between amTreg and nTreg samples within each sex. A Wilcoxon test is applied and adjusted p-values using the Holm method are shown.</em></figcaption>
...
```
<figcaption align = "center"><em class="special">Figure 21: Venn diagram representing the number of shared aaClones between the nTreg samples.</em></figcaption>
<br>
...
<div class="column-left">
```{r dissimilarity, echo = TRUE, fig.cap="Figure 24: A heatmap showing the Morisita distances calculated at the aaClone level between all pairwise samples. The ward D method was used to perform the hierarchical clustering."}
...
<div class="column-right">
```{r mds1, echo = TRUE,warning=FALSE, message=FALSE, fig.cap="Figure 25: Morisita distances were calculated at the aaClone level between all pairwise samples within the dataset and used to perform a multidimensional scaling analysis."}
plotDissimilarity(x = RepSeqData,
                 level = "aaClone",
                 method = "morisita",
                 colorBy = "cell_subset",
...
## Spectratyping comparison
 ...
 `perturbationScore()` function computes the perturbation scores of the aaCDR3 length distribution within each V gene as a distance between each repertoire and the mean repertoire of the control group.
